...
export default function AdminDashboardPage() {
  ...
  return <div className="dashboard-page p-6 md:p-10">
    ...
    <div className="mt-10"><SectionHeading eyebrow="Live now" title="Active elections" href="/admin/elections" linkLabel="Manage elections" />{data.activeElections.length === 0 ? <div className="surface-card"><EmptyState icon={Trophy} title="No active elections" description="Activate an election to start receiving votes." /></div> : <div className="max-h-[clamp(16rem,45vh,32rem)] overflow-y-auto overscroll-contain pr-1 focus-ring"><div className="grid gap-4 lg:grid-cols-3">{data.activeElections.map((election) => <Link key={election.id} href={`/admin/elections/${election.id}/edit`} className="surface-card group p-5 transition hover:border-champagne/35"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-emerald-400/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-300">Active</span><ArrowRight className="h-4 w-4 text-stone transition group-hover:translate-x-0.5 group-hover:text-champagne" /></div><h3 className="mt-5 line-clamp-2 font-display text-lg text-cream">{election.title}</h3><div className="mt-5 grid grid-cols-2 gap-3 border-t border-border/50 pt-4"><div><p className="text-[11px] text-stone">Votes</p><p className="mt-1 font-medium text-cream">{compact(election.totalVotes)}</p></div><div><p className="text-[11px] text-stone">Revenue</p><p className="mt-1 font-medium text-cream">{formatMoney(election.totalRevenue)}</p></div></div><p className="mt-4 flex items-center gap-1.5 text-xs text-stone"><CalendarClock className="h-3.5 w-3.5" />Ends {formatDate(election.endDate)}</p></Link>)}</div></div>}</div>
