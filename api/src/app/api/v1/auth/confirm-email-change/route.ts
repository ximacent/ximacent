// POST /api/v1/auth/confirm-email-change — authenticated.
// Body: { otp }
// On success, email/emailVerified update atomically — this is the only
// moment the address actually changes. From this point on, the new
// email must be used to log in.
...
const postHandler: AuthedHandler = async (req, _ctx) => {
  try {
    const { allowed, resetAt } = checkRateLimit(`auth:confirm-email-change:user:${req.user.sub}`, LIMIT, WINDOW_MS);
    if (!allowed) {
    ...
    const result = await AuthController.confirmEmailChange(req.user.sub, data);
