...
// Body: { newPhone }
// Sends an OTP to newPhone. Does NOT change phone/phoneVerified yet —
// only pendingPhone is set, until confirm-phone-change succeeds.
...
const postHandler: AuthedHandler = async (req, _ctx) => {
  try {
    const { allowed, resetAt } = checkRateLimit(`auth:request-phone-change:user:${req.user.sub}`, LIMIT, WINDOW_MS);
    if (!allowed) {
    ...
    const result = await AuthController.requestPhoneChange(req.user.sub, data);
    return customResponse(SuccessCodes.RECORD_UPDATED.code, "A verification code has been sent to the new phone number.", 200, result);
  } catch (error) {
