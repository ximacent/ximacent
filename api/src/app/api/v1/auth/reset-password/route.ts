...
export async function POST(req: NextRequest) {
  try {
    ...
    const { allowed, resetAt } = checkRateLimit(`auth:reset-password:ip:${ip}`, IP_LIMIT, IP_WINDOW_MS);
    ...
    return customResponse(SuccessCodes.RECORD_UPDATED.code, "Password reset successfully.", 200);
  } catch (error) {
