// POST /api/v1/auth/verify-phone — authenticated. Body: { otp }
...
const postHandler: AuthedHandler = async (req, _ctx) => {
  try {
    const { allowed, resetAt } = checkRateLimit(`auth:verify-phone:user:${req.user.sub}`, LIMIT, WINDOW_MS);
    if (!allowed) {
    ...
    const result = await AuthController.verifyPhoneOTP(req.user.sub, data.otp);
